...
@section('content')
    ...
    <x-adminlte-modal id="deleteModal-{{$PurchasedProducts->id}}" title="Brisanje obroka"  theme="danger"
                      icon="fas fa-trash" v-centered static-backdrop scrollable>
        <form action="{{route('category-destroy', $PurchasedProducts->id)}}" method="post">
            @csrf
            @method('DELETE')
    ...
    <div class="d-flex pt-3">
        <a href="{{route('PurchasedProduct-edit', $PurchasedProducts)}}">
        ...
        <div class="col-md-6">
            <div class="card mb-4 shadow">
                <div class="position-relative">
                    <img src="{{asset($PurchasedProducts->image_url)}}" class="card-img-top rounded-top" alt="Food Image">
                    <div class="overlay"></div>
                    <h5 class="font-weight-bold px-3 pt-3">{{$PurchasedProducts->name}}</h5>
                </div>
                <div class="card-body">
                    
                    <div class="mb-4">
                        <h6 class="card-subtitle mb-2 font-weight-bold">Id korisnika</h6>
                        <p class="card-text">{{$PurchasedProducts->user_id}}</p>
                    </div>
                    <div class="mb-4">
                        <h6 class="card-subtitle mb-2 font-weight-bold">Id proizvoda</h6>
                        <p class="card-text">{{$PurchasedProducts->product_id}}</p>
                    </div>
                </div>
